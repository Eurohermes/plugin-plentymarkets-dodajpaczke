...
class ShippingController extends Controller
{
    ...
    public function __construct(
        ...
        /* Eurohermes configuration */
        $this->ehApiUrl = $config->get('DodajPaczke.global.ehApiUrl');
    ...
    public function getLabels(Request $request, $orderIds)
    {
        ...
        foreach ($orderIds as $orderId) {
            ...
            foreach ($results as $result) {
                ...
                $this->getLogger(__METHOD__)->error("DodajPaczke::logging.exception", ['exists' => $this->storageRepository->doesObjectExist("DodajPaczke", "$labelKey.pdf")]);
                if (
                    !is_null($labelKey) &&
    ...
    private function saveShippingInformation($orderId, $shipmentDate, $shipmentItems)
    {
        ...
        $data = [
            ...
            'shippingServiceProvider' => 'DodajPaczke',
     ...
     */
    private function buildShipmentItems(string $labelUrl, string $shipmentNumber, int $dodajpaczkeShipmentId): array
    {
        return [
            'labelUrl' => $labelUrl,
            'shipmentNumber' => $shipmentNumber,
            'dodajpaczkeShipmentId' => $dodajpaczkeShipmentId
        ];
    }
    ...
    {
        $ch = curl_init("$this->ehApiUrl/users/authentication?login=$this->ehApiLogin&password=$this->ehApiPassword");
    ...
    private function handleCreateRequest(array $requestData): array
    {
        ...
        $ch = curl_init("$this->ehApiUrl/shipments?sync=1");
        ...
        if (is_null($shipmentData)) {
            return [
                'success' => false,
                'status' => 'Shipment has been created in Dodajpaczke but we failed retrieving its\' data.',
            ];
        }

        return [
            'success' => true,
            'status' => 'Shipment has been created in Dodajpaczke service.',
            'shipmentNumber' => $shipmentData['shipmentNumber'],
            'labelUrl' => $shipmentData['labelUrl'],
            'dodajpaczkeShipmentId' => $shipmentData['dodajpaczkeShipmentId']
        ];
    }
    ...
    private function handleCancelRequest($shipmentNumber): array
    {
        ...
        $accessToken = $authRequestHandler['accessToken'];
        $ch = curl_init("$this->ehApiUrl/shipments/$shipmentNumber?operation=cancel");
    ...
    private function retrieveShipmentData(array $response)
    {
        ...
        return [
            ...
            'dodajpaczkeShipmentId' => $shipmentId,
            'labelUrl' => "$this->ehApiUrl/shipments/$shipmentId/shippingLabel"
        ];
    }
